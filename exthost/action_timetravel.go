...
func (a *timeTravelAction) Prepare(_ context.Context, state *TimeTravelActionState, request action_kit_api.PrepareActionRequestBody) (*action_kit_api.PrepareResult, error) {
	if err := requireHeldCapabilities("Time travel attacks", clockCapabilities); err != nil {
		return nil, err
	}
	if extutil.ToBool(request.Config["disableNtp"]) {
		if err := requireCapabilities("Time travel attacks blocking NTP", networkCapabilities); err != nil {
			return nil, err
		}
	}
